...
(define (make-complex-from-real-imag x y)
  ((get 'make-from-real-imag 'complex) (/ x 1.0) (/ y 1.0)))
(define (make-complex-from-mag-ang r a)
  ((get 'make-from-mag-ang 'complex) (/ r 1.0) (/ a 1.0)))
...
(define (apply-generic op . args)
  (define (apply-drop obj)
    (if (equal? (type-tag obj) (car (get-tower-hierarchy)))
	obj
	(let ((drop-result (drop obj)))
  ...
  (let ((type-tags (map type-tag args)))
    (let ((proc (get op type-tags)))
      (if proc
          (apply-drop (apply proc (map contents args)))
...
(drop (drop (drop (make-complex-from-real-imag 20 0))))
;Value: 20.
